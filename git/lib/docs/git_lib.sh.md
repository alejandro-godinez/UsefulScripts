...
|-----------|------|-------------|
| RGX_MAIN |  |  main branch name regex   |
| MAIN_BRANCHES |  |  list of common main branch names   |
| TRIM_SIZE |  |  stash trim size   |
...
| gitStash(repoDir,&nbsp;message,&nbsp;untracked) |  Perform a stash of code   <br><br><u><b>Args:</b></u><br>repoDir - path to local git project  <br>message - optional, message for the stash entry  <br>untracked - optional, true/false flag to include untracked (-u)  <br> |
...
| gitRevisionCounts(repoDir,&nbsp;remote) |  Get revision counts comparing current working branch against the local master   or if you specify the remote orign master.   <br><br><u><b>Args:</b></u><br>repoDir - path to local git project  <br>remote - optional, 'remote' keyword to indicate counts against remote, local otherwise  <br><br><u><b>Output:</b></u><br>two tab separated count numbers, indicating revision ahead and behind  <br> |
| gitBranchList(repoDir,&nbsp;ascending,&nbsp;remote) |  Perform a git branch command to print branches sorted by last commit date in descending   order (newest first), or ascending (oldest first) if option is specified.    <br><br><u><b>Args:</b></u><br>repoDir - path to the local git project  <br>ascending - optional, true/flase flag to indicate ascending order (oldest first)  <br>remote - optional, true/false falag to list remote branches  <br> |
| gitDeleteBranch(repoDir,&nbsp;branchName,&nbsp;force) |  Perform a branch delete command (git branch -d).   <br><br><u><b>Args:</b></u><br>repoDir - path to the local git project  <br>branchName - name of branch to delete  <br>force - optional, true/flase flag to indicate force delete option (git branch -D)  <br> |
