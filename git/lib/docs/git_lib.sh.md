...
| gitBranchList(repoDir,&nbsp;ascending) |  Perform a git branch command to print branches sorted by last commit date in descending   order (newest first), or ascending (oldest first) if option is specified.    <br><br><u><b>Args:</b></u><br>repoDir - path to the local git project  <br>ascending - optional, true/flase flag to indicate ascending order (oldest first)  <br> |
